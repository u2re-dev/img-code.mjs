...
const wIHDR = (width, height) => {
    const chunk = new Uint8Array(25);
    chunk.set(new Uint8Array([0,0,0,13, 0x49,0x48,0x44,0x52]), 0); // length + 'IHDR'
    chunk.set(new Uint8Array([(width>>>24)&255, (width>>>16)&255, (width>>>8)&255, width&255]), 8);
    chunk.set(new Uint8Array([(height>>>24)&255, (height>>>16)&255, (height>>>8)&255, height&255]), 12);
...
/**
 * Оборачивает zlib-буфер в IDAT-чанк PNG
 * @param {Uint8Array} zlibData - данные (zlib+deflate+raw)
 * @returns {Uint8Array} - готовый IDAT-чанк
 */
const wIDAT = (zlibData) => {
    const length = zlibData.length;
    const chunk = new Uint8Array(length + 12);
    chunk.set(new Uint8Array([(length>>>24)&255, (length>>>16)&255, (length>>>8)&255, length&255]), 0);
    chunk.set(new Uint8Array([0x49, 0x44, 0x41, 0x54]), 4);
    chunk.set(zlibData, 8);

    // 4. CRC32 по 'IDAT'+data
    const crc = crc32(chunk.subarray(4, 8 + length));
    chunk.set(new Uint8Array([(crc>>>24)&255, (crc>>>16)&255, (crc>>>8)&255, crc&255]), 8 + length);
    return chunk;
}
